...
def orientation_error_vec(r_des, r_cur):
    # small-angle approximation from rotation matrix skew part
    r_err = r_des @ r_cur.T
    return 0.5 * np.array([
        r_err[2, 1] - r_err[1, 2],
        r_err[0, 2] - r_err[2, 0],
...
class AdmittanceInsertController:
    ...
    def tick(self):
        ...
        # Scene-specific hybrid behavior:
        # - X is drill/feed axis
        # - Y/Z remain compliant via sensed lateral force
        task = (K_POS * pos_err) - (K_FORCE * force)
        # small torque-assisted correction injected into Y/Z components
...
class DrillFeedController:
    ...
    def tick(self):
        ...
        force = self.robot.get_force()
        # crude feed regulation on X axis from measured axial force
        target = self.start_pos.copy()
