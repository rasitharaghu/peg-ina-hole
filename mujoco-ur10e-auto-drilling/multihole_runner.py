...
XML_PATH = "scene_active_frame_multihole.xml"
...
def set_only_active_frame_visible(model, hole_name):
    # Hide all frame geoms by alpha=0, show only current hole
    for hole in HOLES:
...
def move_tip_to_hole(model, data, viewer, control_sid, tip_sid, hole_center, sleep_s):
    ...
    attachment_target = hole_center - current_rot @ tip_offset_local
    ...
    smooth_move_site(model, data, viewer, control_sid, attachment_target, MOVE_DURATION, sleep_s)
...
def main(sleep=DEFAULT_SLEEP):
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        for i, hole in enumerate(HOLES):
            ...
            run_axial_phase(
                model, data, viewer, control_sid,
                depth=0.008, duration=INSERTION_DURATION,
                damping=INSERTION_DAMPING, dt=INSERTION_DT,
                max_travel=INSERTION_MAX_TRAVEL, k_vec=INSERTION_K,
                sleep_s=sleep, label="INSERT"
            )

            print("[DRILL]")
            run_axial_phase(
                model, data, viewer, control_sid,
                depth=0.004, duration=DRILL_DURATION,
                damping=1.0, dt=DRILL_DT,
                max_travel=DRILL_MAX_TRAVEL, k_vec=DRILL_K,
                sleep_s=sleep, label="DRILL"
            )

            if i < len(HOLES) - 1:
                next_center_sid = model.site(HOLES[i + 1]["center_site"]).id
                next_hole_center = get_site_vec(data, next_center_sid)
                print("[TRANSITION] smoothly retracting and moving to next hole")
                retract_and_transition(model, data, viewer, control_sid, tip_sid, next_hole_center, sleep)
