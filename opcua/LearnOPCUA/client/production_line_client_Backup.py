...
class ProductionLineClient:
    ...
    def add_models(self, root):
        """Add Models section when available."""
        models_el = SubElement(root, "Models")
        # Server-side model metadata may not be exposed by the client library.
        # Leave the element empty if no model data is available.
        try:
            if hasattr(self.client, "uaclient") and hasattr(self.client.uaclient, "models"):
                for model in self.client.uaclient.models:
                    model_attribs = {
                        "ModelUri": str(getattr(model, "ModelUri", "")),
                        "PublicationDate": str(getattr(model, "PublicationDate", "")),
                        "Version": str(getattr(model, "Version", "")),
                    }
                    model_el = SubElement(models_el, "Model", model_attribs)
                    for required in getattr(model, "RequiredModel", []):
                        SubElement(model_el, "RequiredModel", {
                            "ModelUri": str(getattr(required, "ModelUri", "")),
                            "PublicationDate": str(getattr(required, "PublicationDate", "")),
                            "Version": str(getattr(required, "Version", "")),
                        })
        except Exception:
            pass
